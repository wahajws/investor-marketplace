    ...
    <DashboardShell title="Founder" navItems={founderNav}>
      ...
      <Card className="mt-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {(recommendations.data?.recommendations ?? ['Create your startup profile.', 'Upload a pitch deck.', 'Add fundraising metrics.']).map((item: string) => (
...
export function FounderCompanyPage() {
  ...
  return (
    <FounderSection title="Company profile" description="Create and maintain the startup profile investors and admins will review." icon={<Building2 size={16} />}>
      ...
      ]} initial={company} onSubmit={async (value) => { company ? await apiPatch(`/companies/${company.id}`, value) : await apiPost('/companies', value); await qc.invalidateQueries({ queryKey: ['companies'] }); }} />
      <Card className="mt-6">
  ...
  return (
    <FounderSection title="AI diligence feedback" description="Run AI extraction, claim checks, red-flag analysis, and founder improvement recommendations. AI output remains preliminary and human-reviewable." icon={<Sparkles size={16} />}>
      {!company ? <EmptyState title="Create a company first" description="AI diligence runs against a startup profile and documents." /> : <>
        <div className="grid gap-4 md:grid-cols-3">
          <MetricCard label="Extractions" value={status.data?.extractions} icon={<FileText size={22} />} />
        ...
        <Card className="mt-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <ActionButton onClick={async () => { await apiPost(`/companies/${company.id}/ai/run-extraction`); await qc.invalidateQueries(); }}><Sparkles size={16} /> Run extraction</ActionButton>
            <ActionButton onClick={async () => { await apiPost(`/companies/${company.id}/ai/run-diligence`); await qc.invalidateQueries(); }}><ShieldCheck size={16} /> Run diligence</ActionButton>
          </div>
        </Card>
    ...
    <DashboardShell title="Investor" navItems={investorNav}>
      ...
      <Card className="mt-6">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          {asArray(pipeline.data).map((item) => <div className="rounded-md border border-[#ded4c4] bg-[#fbfaf6] p-4" key={item.stage}><p className="text-sm text-slate-500">{item.stage}</p><p className="mt-1 text-2xl font-semibold">{item._count}</p></div>)}
        </div>
      </Card>
    </DashboardShell>
  );
